...
export const MenuButton = (props) => {
    return (
        <button className={`${props.className} relative inline-flex items-center justify-center px-4 py-2 overflow-hidden text-white ${props.disabled ? 'bg-primary' : 'bg-lightprimary'} rounded-lg group`}
            onClick={props.onClick} disabled={props.disabled}>
            {props.disabled ? null : <span className="absolute w-0 h-0 transition-all duration-500 ease-out bg-[#24a0ed] rounded-full group-hover:w-56 group-hover:h-56"></span>}
