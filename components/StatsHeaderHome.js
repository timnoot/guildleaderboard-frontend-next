...
export const GuildStatsHeader = ({ stats, title, backButton = false }) => {
  return (
    <header>
      <Head>
        <title>Hypixel SkyBlock {title} Leaderboard</title>
        <meta name='title' content={`Hypixel SkyBlock ${title} Leaderboard`} />
        <meta
          name='description'
...
${spaces(3)}🥉 ${stats.top_guilds[2].guild_name} 🏆 ${numberWithCommas(
      ...
      <h1 className='text-2xl text-center text-white bg-secondary sm:text-5xl mx-2'>
        Hypixel SkyBlock {title} Leaderboard
      </h1>
