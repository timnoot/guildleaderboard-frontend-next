...
import { StatBlockTop, OutsideLink, CopyButton, BackButton, MenuButton } from '../../components/StatBlocks';
...
class JoinLogs extends React.Component {
    ...
    render() {
        ...
        if (this.state.last_page > 4) {
            ...
            } else if (this.state.current_page >= 4 && this.state.current_page <= this.state.last_page - 3) {
                middleButtonsNumbers = [1, this.state.current_page - 1, this.state.current_page, this.state.current_page + 1, this.state.last_page]
            } else if (this.state.current_page >= this.state.last_page - 2) {
                middleButtonsNumbers = [1, this.state.last_page - 4, this.state.last_page - 3, this.state.last_page - 2, this.state.last_page - 1, this.state.last_page]
            }
        } else {
        ...
        for (let i = 0; i < middleButtonsNumbers.length; i++) {
            ...
            if (number - middleButtonsNumbers[i - 1] > 1) {
                middleButtons.push(
                    <span className='text-white py-1 px-1 xs:px-2 my-1 mx-[0.125rem] align-middle' key={`${number}-dot`}>
                        ...
                    </span>
                )
            }
            ...
            middleButtons.push(
                <button className={`${active} rounded-md py-1 px-1 xs:px-2 my-1 mx-[0.125rem] align-middle inline-block`} key={`${number}-button`} onClick={() => {
                ...
                <div className='pt-3'>
                    <div className='bg-tertiary p-2 md:p-6 text-white rounded-md w-11/12 xl:w-2/3 inline-flex text-xs xs:text-sm md:text-xl'>
                        <div className='text-left align-middle' style={{ flexGrow: 1 }}>
                            <button className={`bg-primary rounded-md p-2 ${allow_previous ? 'cursor-pointer border-white border-2' : 'cursor-not-allowed bg-lightprimary text-gray-400'}`} onClick={() => {
                                if (allow_previous) {
                        ...
                        <div className='justify-end text-right align-middle ' style={{ flexGrow: 1 }}>
                            <button className={`bg-primary  rounded-md p-2 ${allow_next ? 'cursor-pointer border-white border-2' : 'cursor-not-allowed bg-lightprimary text-gray-400'}`} onClick={() => {
                                if (allow_next) {
                                    this.loadHistory(this.uuid, this.state.current_page + 1)
                                }
                            }}>
...
class PlayerMetrics extends React.Component {
    ...
    getMetricName(metric) {
        return metric.replace("xp", "XP").split('_').map((s) => s.charAt(0).toUpperCase() + s.substring(1)).join(' ');
    }
    ...
    render() {
        ...
        for (const key in this.state.showMetrics) {
            ...
            if (this.state.showMetrics[key]) {
                ...
                metricsCharts.push(
                    <div>
                        <button className='text-white p-2 m-2 rounded-md w-[90%] md:inline-block h-80 md:h-96 md:w-2/3 bg-primary text-xl md:text-4xl lg:text-6xl' onClick={() => {
            ...
            <div className='text-black'>
                ...
                <div className='text-center text-white text-sm'>
                    <MenuButton
                        onClick={() => {
                            this.setState({
                                daysShow: 7, change: this.state.change + 1, showMetrics: {
                                    general: true,
                                    skill: false,
                                    dungeon: false,
                                    slayer: false,
                    ...
                    <MenuButton
                        onClick={() => {
                            this.setState({
                                daysShow: 30, change: this.state.change + 1, showMetrics: {
                                    general: true,
                                    skill: false,
                                    dungeon: false,
                                    slayer: false,
                    ...
                    <MenuButton
                        onClick={() => {
                            this.setState({
                                daysShow: 90, change: this.state.change + 1, showMetrics: {
                                    general: true,
                                    skill: false,
                                    dungeon: false,
                                    slayer: false,
...
class PlayerStats extends React.Component {
    ...
    render() {
        ...
        for (const skill in skills) {
            ...
            skillProgressBars.push(
                <div className="inline-block p-2 w-[45%] sm:w-64 lg:w-96">
                    <ProgressBar key={skillName} name={`${capitalizeFirstLetter(skillName)} ${numberWithCommas(this.playerJson[skillName])}`}
                        xp={this.playerJson[`${skillName}_xp`]} levelProgress={levelProgress}
                        color={level >= skillMaxLevel[skillName] ? 'bg-progressgold' : 'bg-progressblue'}
                    />
                </div>
            )
        }
        ...
        for (const slayer in slayers) {
            ...
            let { level, progress } = getSlayerLevel(slayerName, this.playerJson[`${slayerName}_xp`])
            if (level >= 9) {
                progress = 1;
            }
            ...
            slayerProgressBars.push(
                <div className="inline-block p-2 w-[45%] sm:w-64 lg:w-96">
                    <ProgressBar key={slayerName} name={`${capitalizeFirstLetter(slayerName)} ${level}`}
                        xp={this.playerJson[`${slayerName}_xp`]} levelProgress={progress}
        ...
        for (const dungeon in dungeons) {
            ...
            dungeonProgressBars.push(
                <div className="inline-block p-2 w-[45%] sm:w-64 lg:w-96">
                    <ProgressBar key={dungeonName} name={`${capitalizeFirstLetter(dungeonName)} ${numberWithCommas(this.playerJson[dungeonName])}`}
                        xp={this.playerJson[`${dungeonName}_xp`]} levelProgress={levelProgress}
...
💀 Catacombs: ${numberWithCommas(this.props.player.catacombs)}
🔫 Slayer: ${numberWithCommas(this.props.player.wolf_xp + this.props.player.spider_xp + this.props.player.zombie_xp + this.props.player.blaze_xp + this.props.player.enderman_xp)}`}
                    />
