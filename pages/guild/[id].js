...
import { StatBlockTop, BackButton, MenuButton } from '../../components/StatBlocks';
...
const Player = (props) => {
    ...
    useEffect(() => { SetTimeAgo(TimeDelta.fromDate(player_data.capture_date).toNiceString()); }, [])

    return (
        <>
            <tr
                className={`${props.color} font-[Helvetica] hover:opacity-60 hover:cursor-pointer`}
                ref={ref} onClick={() => goRouteId(player_data.name)}
            >
                ...
                <th className='sm:px-5 hidden lg:table-cell'>
                    {TimeAgo}
                </th>
                <th></th>
            </tr>
...
class JoinLogs extends React.Component {
    ...
    render() {
        ...
        for (const i in historyData) {
            ...
            logsElements.push(
                <Link href={`/player/${log.uuid}`} key={`${log.uuid}-${log.capture_date}`}>
                    <a>
                        <JoinLog
                            type={log.type}
                            name={log.name}
        ...
        return (
            <div>
                ...
                <div className='text-center text-white text-sm'>
                    <MenuButton
                        onClick={() => { this.setState({ daysShow: 7, change: this.state.change + 1 }) }}
                        disabled={daysShow === 7}
                        className='mx-1'
                    >
                        7 days
                    ...
                    <MenuButton
                        onClick={() => { this.setState({ daysShow: 30, change: this.state.change + 1 }) }}
                    ...
                    <MenuButton
                        onClick={() => { this.setState({ daysShow: 90, change: this.state.change + 1 }) }}
...
export const getServerSideProps = async (context) => {
    const guild = await axios.get(`${APIURL}guild/${context.params?.id}`).then(res => res.data)
