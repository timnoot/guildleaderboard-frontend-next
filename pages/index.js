...
const Guild = (props) => {
	...
	if (Boolean(guildJson.position_change) && props.sortOn === "senither_weight" && !props.sortReversed) {
		position_change = <th className="text-left md:px-1">{positionChange(guildJson.position_change)}</th>
	} else {
	...
	return (
		...
		>
			...
			<th>
				<Tippy
					content={`${numberShortener(guildJson.networth * guildJson.members)} total guild networth`}
				>
